...
MAPE is defined as a summation of forecast errors, where each error is calculated based on the actual load. This is shown in Equation \ref{eq:MAPE} where $y$ represents.....$\hat{y}$ is the....$N$ is the number of
...
Therefore, the forecast error constitutes on higher average forecast errors for the DAF than TSF. With regards to the lock-down effects, the higher forecast accuracy for the DAF is especially remarkable when distilling the time frame on the weekdays from Wednesday to Friday which is time frame when the lock-down was initiated. The typical working week differs from the lock-down working week, and the weekend is more similar to the lock-down weekend. Hence, the effect of the lock-down would be overlooked if the analysed time frame was for the whole weak. On the contrary, no such effects are observed for TSF, which implies that the shorter-length forecast is less subjected to the lock-down effect. 
